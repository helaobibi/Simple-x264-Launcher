	...
	ui(new Ui::MainWindow())
{
	...
	ui->jobsView->horizontalHeader()->setSectionResizeMode(0, QHeaderView::Stretch);
	ui->jobsView->horizontalHeader()->setSectionResizeMode(1, QHeaderView::ResizeToContents);
	ui->jobsView->horizontalHeader()->setSectionResizeMode(2, QHeaderView::ResizeToContents);
	ui->jobsView->horizontalHeader()->setMinimumSectionSize(96);
	ui->jobsView->verticalHeader()->setSectionResizeMode(QHeaderView::ResizeToContents);
...
void MainWindow::shutdownComputer(void)
{
	...
	const Qt::WindowFlags flags = Qt::WindowStaysOnTopHint | Qt::CustomizeWindowHint | Qt::WindowTitleHint | Qt::WindowSystemMenuHint;
...
void MainWindow::init(void)
{
	...
	{
		QMessageBox::critical(this, tr("Invalid File!"), tr("<nobr>At least one required tool is missing or is not executable:</nobr><br><tt>%1</tt><br><br><nobr>Please re-install the program in order to fix the problem!</nobr>").replace("-", "&minus;").arg(QDir::toNativeSeparators(failedPath).toHtmlEscaped()));
		qFatal("At least one required tool is missing or not executable. Program will exit now!");
	}
	qDebug(" ");
...
{
	const MUtils::ArgumentMap &args = MUtils::OS::arguments();
