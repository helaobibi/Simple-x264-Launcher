	...
	if(role == Qt::DisplayRole)
	{
		if(index.row() >= 0 && index.row() < m_jobs.count())
		{
			...
			case 1:
				...
				break;
			case 2:
				return QString::asprintf("%d%%", m_progress.value(m_jobs.at(index.row())));
				break;
			case 3:
...
QModelIndex JobListModel::insertJob(EncodeThread *thread)
{
	...
	const QStringList encoderNameParts = encoderInfo.getName().simplified().split(' ', Qt::SkipEmptyParts);
	...
	return createIndex(m_jobs.count() - 1, 0, nullptr);
}
...
size_t JobListModel::saveQueuedJobs(void)
{
	...
	for(QList<QUuid>::ConstIterator iter = m_jobs.constBegin(); iter != m_jobs.constEnd(); iter++)
	{
		if(m_status.value(*iter) == JobStatus_Enqueued)
		{
			...
			{
				settings.beginGroup(QString::asprintf(JOB_TEMPLATE, jobCounter++));
				...
				settings.setValue(KEY_ENTRY_COUNT, static_cast<quint64>(jobCounter));
			}
		}
	}
...
size_t JobListModel::loadQueuedJobs(const SysinfoModel *sysinfo)
{
	...
	{
		if(!groups.contains(QString::asprintf(JOB_TEMPLATE, i)))
		{
			return 0;
		}
	}
	...
	for(size_t i = 0; i < jobCounter; i++)
	{
		settings.beginGroup(QString::asprintf(JOB_TEMPLATE, i));
