...
void PreferencesDialog::emulateMouseEvent(QObject *object, QEvent *event, QWidget *source, QWidget *target)
{
	if(object == source)
	{
		...
		{
			const QPointF localPos = (qApp->widgetAt(mouseEvent->globalPosition().toPoint()) == source) ? QPointF(1, 1) : QPointF(INT_MAX, INT_MAX);
...
void PreferencesDialog::done(int n)
{
	...
	m_preferences->setPrefer64BitSource (false);
