...
#define X264_UPDATE_PROGRESS(M) do \
{ \
...
public:
	...
	virtual QString getBinaryPath(const SysinfoModel *sysinfo, const quint32 &encArch, const quint32 &encVariant) const
	{
		...
		Q_UNUSED(encArch);
		Q_UNUSED(encVariant);
		return QString("/opt/homebrew/bin/x264");
	}
...
X264Encoder::X264Encoder(JobObject *jobObject, const OptionsModel *options, const SysinfoModel *const sysinfo, const PreferencesModel *const preferences, JobStatus &jobStatus, std::atomic<bool> *abort, std::atomic<bool> *pause, QSemaphore *semaphorePause, const QString &sourceFile, const QString &outputFile)
:
...
void X264Encoder::checkVersion_init(QList<QRegularExpression*> &patterns, QStringList &cmdLine)
{
	cmdLine << "--version";
	patterns << new QRegularExpression("\\bx264\\s+(\\d)\\.(\\d+)\\.(\\d+)\\s+([a-f0-9]{7})", QRegularExpression::CaseInsensitiveOption);
	patterns << new QRegularExpression("\\bx264\\s+(\\d)\\.(\\d+)\\.(\\d+)", QRegularExpression::CaseInsensitiveOption);
}

void X264Encoder::checkVersion_parseLine(const QString &line, const QList<QRegularExpression*> &patterns, unsigned int &core, unsigned int &build, bool &modified)
{
	QRegularExpressionMatch match;
	if((match = patterns[0]->match(line)).hasMatch())
	{
		bool ok1 = false, ok2 = false;
		unsigned int temp1 = match.captured(2).toUInt(&ok1);
		unsigned int temp2 = match.captured(3).toUInt(&ok2);
		if(ok1 && ok2)
		{
			core  = temp1;
			build = temp2;
...
void X264Encoder::runEncodingPass_init(QList<QRegularExpression*> &patterns)
{
	patterns << new QRegularExpression("\\[(\\d+)\\.(\\d+)%\\].+frames");
	patterns << new QRegularExpression("indexing.+\\[(\\d+)\\.(\\d+)%\\]");
	patterns << new QRegularExpression("^(\\d+) frames:");
	patterns << new QRegularExpression("\\[\\s*(\\d+)\\.(\\d+)%\\]\\s+(\\d+)/(\\d+)\\s(\\d+).(\\d+)\\s(\\d+).(\\d+)\\s+(\\d+):(\\d+):(\\d+)\\s+(\\d+):(\\d+):(\\d+)"); //regExpModified
}

void X264Encoder::runEncodingPass_parseLine(const QString &line, const QList<QRegularExpression*> &patterns, const ClipInfo &clipInfo, const int &pass, double &last_progress, double &size_estimate)
{
	int offset = -1;
	QRegularExpressionMatch match;
	if((match = patterns[0]->match(line)).hasMatch())
