...
bool AbstractSource::checkSourceProperties(ClipInfo &clipInfo)
{
	...
	if(bTimeout || bAborted || process.exitCode() != EXIT_SUCCESS)
	{
		if(!(bTimeout || bAborted))
		{
			...
			{
				log(tr("\nIMPORTANT: The input process terminated abnormally. This means VapourSynth or one of its plugins crashed!"));
			}
		}
		return false;
	}
...
bool AbstractSource::createProcess(QProcess &processEncode, QProcess&processInput)
{
	...
	const QStringList extraPathsIn = getExtraPaths();
	const QHash<QString, QString> extraEnvIn = getExtraEnv();
	if(!startProcess(processInput, getBinaryPath(), cmdLine_Input, false, &extraPathsIn, &extraEnvIn))
	{
		return false;
	}
