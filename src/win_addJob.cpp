...
void AddJobDialog::accept(void)
{
	...
	if(sourceType == MediaInfo::FILETYPE_VAPOURSYNTH)
	{
	...
	else if(QObject::sender() == ui->buttonBrowseOutput)
	{
		...
		if(!(filePath.isNull() || filePath.isEmpty()))
		{
			if(getFilterIdx(QFileInfo(filePath).suffix()) < 0)
			{
				...
				QRegularExpressionMatch regMatch = regExp.match(selectedType);
				if(regMatch.hasMatch())
				{
					tempIndex = getFilterIdx(regMatch.captured(1));
				}
				if(tempIndex < 0)
				{
