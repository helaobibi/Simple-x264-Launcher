...
unsigned int AbstractTool::checkVersion(bool &modified)
{
	...
	log(tr("Binary path: %1").arg(getBinaryPath()));
	log(tr("Channel mode: MergedChannels (stdout+stderr)"));
	const QStringList extraPaths = getExtraPaths();
	const QHash<QString, QString> extraEnv = getExtraEnv();
	if(!startProcess(process, getBinaryPath(), cmdLine, true, &extraPaths, &extraEnv))
	{
		log("ERROR: Failed to create process!");
		return false;
	}
	log(tr("Process started successfully, PID: %1").arg(process.processId()));
	...
	{
		log(tr("Process finished, reading remaining output (bytes available: %1)...").arg(process.bytesAvailable()));
		PROCESS_PENDING_LINES(process, checkVersion_parseLine, patterns, coreVers, revision, modified);
	}
	...
	{
		QRegularExpression *pattern = patterns.takeFirst();
		MUTILS_DELETE(pattern);
	}

	log(tr("\nProcess exited with code: %1 (timeout=%2, aborted=%3)").arg(QString::number(process.exitCode()), bTimeout ? "yes" : "no", bAborted ? "yes" : "no"));
	...
	log(tr("Parsed version values: coreVers=%1, revision=%2, modified=%3").arg(QString::number(coreVers), QString::number(revision), modified ? "yes" : "no"));

	if((revision == UINT_MAX) || (coreVers == UINT_MAX))
	{
		log(tr("\nFAILED TO DETERMINE VERSION INFO !!!"));
		log(tr("Hint: No output line matched the expected version pattern."));
		return UINT_MAX;
	}
