...
HelpDialog::HelpDialog(QWidget *parent, bool sourceMode, const SysinfoModel *const sysinfo, const OptionsModel *const options, const PreferencesModel *const preferences)
:
	QDialog(parent),
...
{
	ui->logo_x264->setHidden(m_sourceMode);
	...
	{
		m_process->start(SourceFactory::getSourceInfo(SourceFactory::SourceType_VPS).getBinaryPath(m_sysinfo, true), QStringList() << "--version");
	}
