...
static void READ_INT(QSettings &settings, const QString &key, int defaultVal, int *value)
{
	bool ok = false;
	const int temp = settings.value(key, defaultVal).toInt(&ok);
	*value = (ok) ? temp : defaultVal;
}

static QString moviesLocation(void)
{
	static const QStandardPaths::StandardLocation locations[3] =
	{
		QStandardPaths::MoviesLocation, QStandardPaths::DesktopLocation, QStandardPaths::HomeLocation
	};

	for(size_t i = 0; i < 3; i++)
	{
		const QString path = QDir::fromNativeSeparators(QStandardPaths::writableLocation(locations[i]));
		if(!path.isEmpty())
		{
			QDir directory(path);
			if(!directory.exists())
