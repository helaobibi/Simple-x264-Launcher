...
bool AbstractEncoder::runEncodingPass(AbstractSource* pipedSource, const QString outputFile, const ClipInfo &clipInfo, const int &pass, const QString &passLogFile)
{
	...
	if(!startProcess(processEncode, getBinaryPath(), cmdLine_Encode, true, &extraPaths, &extraEnv))
	{
		return false;
	}
		...
		forever
		{
			...
			if((*m_pause) && (processEncode.state() == QProcess::Running))
			{
				...
				QProcess *proc[2] = { &processEncode, &processInput };
					MUtils::OS::suspend_process(proc[0], true);
				MUtils::OS::suspend_process(proc[1], true);
				while(*m_pause) m_semaphorePause->tryAcquire(1, 5000);
				while(m_semaphorePause->tryAcquire(1, 0));
				MUtils::OS::suspend_process(proc[0], false);
				MUtils::OS::suspend_process(proc[1], false);
...
QString AbstractEncoder::sizeToString(qint64 size)
{
	...
	if(size > 1024LL)
	{
		...
		qint64 remainderSize = 0LL;

		int prefixIdx = 0;
		while((estimatedSize > 1024LL) && (prefixIdx < 4))
		{
			remainderSize = estimatedSize % 1024LL;
			estimatedSize = estimatedSize / 1024LL;
			prefixIdx++;
		}
			
		double value = static_cast<double>(estimatedSize) + (static_cast<double>(remainderSize) / 1024.0);
		return QString::asprintf((value < 10.0) ? "%.2f %s" : "%.1f %s", value, prefix[prefixIdx]);
	}
